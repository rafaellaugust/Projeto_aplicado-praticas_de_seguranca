...
if (isset($_GET['action']) && $_GET['action'] === 'cobrar_wa' && !empty($_GET['id'])) {
    ...
    if ($fat) {
        ...
        // Sucesso completo = corpo enviado E (sem PIX aplicável OU PIX enviado com sucesso)
        $sucessoCompleto = $res['corpo'] && ($res['pix'] === null || $res['pix'] === true);

        if ($sucessoCompleto) {
            $db->exec("UPDATE faturas SET notificado_wa = 1 WHERE id = " . $id);
            if (!empty($res['pix_copia_cola'])) {
                $msgSuccess = "Cobrança e chave PIX enviadas com sucesso no WhatsApp de {$fat['nome']}!";
            } else {
                $msgSuccess = "Cobrança enviada com sucesso no WhatsApp de {$fat['nome']}! (Sem PIX disponível para envio)";
            }
        } else {
                ...
                <?php else: ?>
                    <?php foreach ($faturas as $f): ?>
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-sm btn-outline-info" title="Ver Fatura / PIX"
                                        onclick="abrirModalPix(<?= (int)$f['id'] ?>, '<?= htmlspecialchars(addslashes(sanitize($f['cliente_nome'])), ENT_QUOTES) ?>', '<?= htmlspecialchars(formatMoeda($f['valor']), ENT_QUOTES) ?>', '<?= htmlspecialchars(formatData($f['data_vencimento']), ENT_QUOTES) ?>', '<?= $f['status'] ?>', <?= json_encode($f['pix_copia_cola'] ?? '') ?>)">
        ...
        <div class="modal-content bg-dark text-light border-secondary">
            ...
            <div class="modal-footer border-secondary">
                ...
                <a id="pixModalLinkCompleto" href="#" target="_blank" class="btn btn-outline-info">
                    <i class="fa-solid fa-arrow-up-right-from-square me-1"></i>Abrir Fatura Completa
                </a>
            </div>
        </div>
    </div>
...
<script>
function abrirModalPix(faturaId, cliente, valor, vencimento, status, pixCopiaCola) {
    ...
    document.getElementById('pixModalLinkCompleto').href = '../cliente/fatura.php?id=' + faturaId;
    ...
    if (status === 'pago') {
        ...
        canvas.innerHTML = '';
        new QRCode(canvas, { text: pixCopiaCola, width: 220, height: 220 });
    }

    const modal = new bootstrap.Modal(document.getElementById('modalVerPix'));
    modal.show();
}
...
function copiarPixModal() {
    ...
    navigator.clipboard.writeText(code).then(() => {
        ...
        setTimeout(() => { btn.innerHTML = orig; btn.classList.replace('btn-success', 'btn-primary-custom'); }, 2500);
    });
}
</script>

        ...
        <table class="table-custom">
            ...
            <tbody>
                <?php foreach($clientesLista as $c):
                    ...
                    if($planoAtual){
                        $nomeAtual = $planoAtual['nome'];
                        $parts = explode('-', $nomeAtual);
                        if(count($parts) === 3) {
                            $speed = $parts[1];  // ex: 40Megas
                            $suffix = $parts[2]; // ex: 20
                            $buscaS1 = "S1-" . $speed . "-" . $suffix;
                            $buscaS2 = "S2-" . $speed . "-" . $suffix;
                            foreach($planosDisponiveis as $pl){
                                if(strcasecmp($pl['nome'], $buscaS1) === 0) $s1 = $pl;
                                if(strcasecmp($pl['nome'], $buscaS2) === 0) $s2 = $pl;
                            }
                        }
                        if(!$s1 || !$s2) {

    ...
    if (!empty($selecionados)) {
        foreach ($selecionados as $username) {
            ...
            if ($secretTarget) {
                ...
                $waFicticio = '55119' . sprintf('%08d', rand(10000000, 99999999));

                try {
                    $stmtIns = $db->prepare("
                        INSERT INTO clientes (nome, cpf_cnpj, whatsapp, pppoe_usuario, pppoe_senha, plano_id, status, sincronizado_mikrotik) 
                        VALUES (?, ?, ?, ?, ?, ?, 'ativo', 1)
                        ON DUPLICATE KEY UPDATE pppoe_senha = VALUES(pppoe_senha), plano_id = COALESCE(VALUES(plano_id), plano_id), sincronizado_mikrotik = 1
                    ");
                    $stmtIns->execute([$user, $cpfFicticio, $waFicticio, $user, $pass, $planoId]);
                    $importados++;
                } catch (Exception $e) {
                }
            }

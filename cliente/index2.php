...
<style>
.stat-card { background: #1e293b; border: 1px solid #334155; border-radius: 16px; transition: all 0.3s; }
...
<div class="row mb-4">
    <div class="col-md-8">
        ...
        <p class="text-secondary small mb-0">Portal do Assinante — <?= sanitize($cliente['plano_nome'] ?? 'Sem Plano') ?></p>
    </div>
    <div class="col-md-4 text-md-end mt-2 mt-md-0">
        <span class="badge bg-dark border border-secondary px-3 py-2"><i class="fa-solid fa-wifi me-1 text-success"></i><?= sanitize($cliente['pppoe_usuario']) ?></span>
    </div>
</div>
...
<div class="card-custom mb-4 border <?= $statusCaixa === 'overdue' ? 'border-danger' : 'border-warning' ?> border-opacity-50"
    ...
    <div class="row align-items-center">
        ...
        <div class="col-md-5 text-md-end mt-3 mt-md-0">
            <a href="fatura.php?id=<?= $faturaAberta['id'] ?>" class="btn <?= $statusCaixa === 'overdue' ? 'btn-danger' : 'btn-warning' ?> btn-lg rounded-3 fw-bold text-dark px-4">
...
<div class="row g-4">
    ...
    <div class="col-md-5">
        ...
        <div class="card-custom bg-dark border border-info border-opacity-25">
            <h6 class="text-info fw-bold"><i class="fa-solid fa-circle-info me-2"></i>Seu Plano</h6>
            <p class="mb-1 text-white fw-bold"><?= sanitize($cliente['plano_nome'] ?? 'N/A') ?></p>
            <p class="mb-1 small text-secondary">Download: <?= sanitize($cliente['velocidade_down'] ?? '?') ?> / Upload: <?= sanitize($cliente['velocidade_up'] ?? '?') ?></p>
            <p class="mb-0 small text-secondary">Usuário: <code class="text-info"><?= sanitize($cliente['pppoe_usuario']) ?></code></p>
            <hr class="border-secondary">
            <p class="mb-0 small text-secondary">
                <i class="fa-solid fa-circle-info me-1 text-info"></i>
                Ao pagar, aguarde alguns segundos para seu acesso ser liberado automaticamente.
            </p>
        </div>
    </div>
</div>

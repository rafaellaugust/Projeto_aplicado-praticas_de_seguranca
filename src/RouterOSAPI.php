...
class RouterosAPI
{
    ...
    public function connect($ip, $login, $password)
    {
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            ...
            if ($this->certless) {
                $sslOptions['ciphers'] = 'ADH:ALL:@SECLEVEL=0';
            }
            ...
            if ($this->socket) {
                ...
                $this->debug('Login response: ' . print_r($RESPONSE, true));
                
                if (isset($RESPONSE[0])) {
                    if ($RESPONSE[0] == '!done') {
                        if (!isset($RESPONSE[1])) {
                            // Login method post-v6.43
                            $this->debug('Authentication successful (post-v6.43 method)');
                            $this->connected = true;
                            break;
                        } else {
                            // Login method pre-v6.43
                            $this->debug('Using pre-v6.43 authentication method');
                            $MATCHES = array();
                            if (preg_match_all('/[^=]+/i', $RESPONSE[1], $MATCHES)) {
                                if ($MATCHES[0][0] == 'ret' && strlen($MATCHES[0][1]) == 32) {
                                    $this->debug('Challenge received: ' . $MATCHES[0][1]);
                                    $this->write('/login', false);
                                    $this->write('=name=' . $login, false);
                                    $this->write('=response=00' . md5(chr(0) . $password . pack('H*', $MATCHES[0][1])));
                                    $RESPONSE = $this->read(false);
                                    $this->debug('Challenge response: ' . print_r($RESPONSE, true));
                                    if (isset($RESPONSE[0]) && $RESPONSE[0] == '!done') {
                                        $this->debug('Authentication successful (pre-v6.43 method)');
                                        $this->connected = true;
                                        break;
                                    } else {
                                        $this->debug('Challenge authentication failed');
                                    }
                                }
                            }
                        }
                    } else if ($RESPONSE[0] == '!trap') {
                        ...
                        $this->debug('Authentication error: ' . $errorMsg);
                        $this->error_str = $errorMsg;
                    }
                }
                fclose($this->socket);
            }
